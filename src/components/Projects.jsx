...
const financeScreenshot = `${import.meta.env.BASE_URL}finanical.jpeg`;
const leavemsScreenshot = `${import.meta.env.BASE_URL}leavems.png`;
const todoScreenshot = `${import.meta.env.BASE_URL}todo.png`;
const galleryScreenshot = `${import.meta.env.BASE_URL}gallery.png`;
...
const projects = [
    ...
    screenshots: [
      {
        ...
        image: financeScreenshot,
        gradient: "from-green-900 via-emerald-900 to-teal-900",
        icon: "🧮",
      },
      {
        label: "SIP Planner",
        gradient: "from-teal-900 via-green-900 to-cyan-900",
        icon: "📈",
      },
      {
  ...
  {
    title: "Java Todo CLI — Docker + CI/CD",
    emoji: "🐳",
    tagColor: "bg-purple-500/20 text-purple-300 border-purple-500/30",
    accentColor: "text-purple-400",
    problem:
      "Manual builds, inconsistent environments across machines, and zero deployment automation make even simple projects painful to ship and maintain reliably.",
    solution:
      "Built a console-based Java Todo application, containerized it with Docker for total environment consistency, then wired a Jenkins + Maven CI/CD pipeline that automatically builds, tests, and packages on every commit.",
    techStack: ["Java", "Docker", "Jenkins", "Maven", "CI/CD"],
    features: [
      "Full CRUD task management via command-line interface",
      "Custom Dockerfile — portable, reproducible container image",
      "Jenkins declarative pipeline: build → test → package on every push",
      "Maven handles dependency management and lifecycle automation",
      "One-command deployment from any machine: docker run",
    ],
    screenshots: [
      {
        label: "CLI Interface",
  ...
  },
  {
    ...
    screenshots: [
      {
        label: "Dashboard",
        image: leavemsScreenshot,
        gradient: "from-blue-900 via-indigo-900 to-blue-900",
        icon: "📋",
      },
      {
        label: "Apply Leave",
        gradient: "from-indigo-900 via-blue-900 to-sky-900",
        icon: "📝",
      },
      {
        label: "Admin Panel",
  ...
  return (
    ...
    >
      ...
      >
        {/* Main image */}
        <div
          className={`bg-linear-to-br ${shot.gradient} rounded-2xl aspect-video flex flex-col items-center justify-center shadow-2xl`}
        >
          {shot.image ? (
            <img
              src={shot.image}
          ...
          <div className="flex justify-center gap-2 mt-4">
            {screenshots.map((_, i) => (
              <button
                ...
                onClick={() => {
                  /* handled via onPrev/onNext pattern — dots are visual only here */
                }}
...
function ScreenshotGallery({ screenshots, accentColor }) {
  const galleryShots = screenshots.slice(0, 1);
  const [lightbox, setLightbox] = useState(null); // null | index

  const close = useCallback(() => setLightbox(null), []);
  const prev = useCallback(
    () =>
      setLightbox((i) => (i - 1 + galleryShots.length) % galleryShots.length),
    [galleryShots.length],
  );
  const next = useCallback(
    () => setLightbox((i) => (i + 1) % galleryShots.length),
    [galleryShots.length],
  );
  ...
  return (
    <>
      <div className="mb-5">
        <h4
          className={`text-xs font-bold uppercase tracking-widest mb-3 ${accentColor}`}
        >
          Screenshots
        </h4>
            ...
            >
              {shot.image ? (
                ...
                <>
                  ...
                  <div
                    className={`absolute inset-0 bg-linear-to-br ${shot.gradient} transition-transform duration-500 group-hover/thumb:scale-110`}
                  />
                  {/* Icon overlay */}
...
function ProjectCard({ project, theme }) {
  ...
  return (
    <div
      className={`border rounded-3xl p-7 hover:-translate-y-1 hover:shadow-2xl hover:shadow-purple-900/25 hover:border-purple-500/40 transition-all duration-300 flex flex-col group ${isDark ? "bg-gray-800/60 border-gray-700/80" : "bg-gray-100 border-gray-200"}`}
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-5">
        <span className="text-3xl">{project.emoji}</span>
        <h3
          className={`text-xl font-bold group-hover:text-purple-300 transition-colors tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}
        >
          {project.title}
        </h3>
      </div>
      ...
      <div className="mb-4">
        <h4
          className={`text-xs font-bold uppercase tracking-widest mb-2 ${project.accentColor}`}
        >
          Problem
        </h4>
        <p
          className={`text-sm leading-relaxed ${isDark ? "text-gray-400" : "text-gray-700"}`}
        >
          {project.problem}
        </p>
      </div>
      ...
      <div className="mb-6 flex-1">
        <h4
          className={`text-xs font-bold uppercase tracking-widest mb-3 ${project.accentColor}`}
        >
          Key Features
        </h4>
        <ul className="space-y-2">
          {project.features.map((feature) => (
            <li
              key={feature}
              className={`flex items-start gap-2 text-sm ${isDark ? "text-gray-400" : "text-gray-700"}`}
            >
      ...
      <div className="flex gap-3 mt-auto">
        <a
          ...
          className={`flex-1 text-center py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 flex items-center justify-center gap-2 border ${isDark ? "bg-gray-700/80 hover:bg-gray-600 text-white border-gray-600/50 hover:border-gray-500" : "bg-gray-200 hover:bg-gray-300 text-gray-900 border-gray-300 hover:border-gray-400"}`}
        >
